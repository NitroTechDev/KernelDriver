...
bool ChecdkBox(int x, int y, bool var, POINT Mouse) {
	if (Mouse.x >= x && Mouse.x <= x + 111 && Mouse.y >= y && Mouse.y <= y + 10) {
		var = !var;
		return var;
...
bool ChaeckBox(int x, int y, bool var, POINT Mouse) {
	if (Mouse.x >= x && Mouse.x <= x + 111 && Mouse.y >= y && Mouse.y <= y + 10) {
		var = !var;
		return var;
...
bool CshechBoxz(int x, int y, bool var, POINT Mouse) {
...
bool qdqwfff(int x, int y, bool var, POINT Mouse) {
	if (Mouse.x >= x && Mouse.x <= x + 159 && Mouse.y >= y && Mouse.y <= y + 10) {
		var = !var;
		return var;
...
bool dqddd(int x, int y, bool var, POINT Mouse) {
	if (Mouse.x >= x && Mouse.x <= x + 159 && Mouse.y >= y && Mouse.y <= y + 10) {
		var = !var;
		return var;
...
bool CshecqdqdqhBoxz(int x, int y, bool var, POINT Mouse) {
	if (Mouse.x >= x && Mouse.x <= x + 159 && Mouse.y >= y && Mouse.y <= y + 10) {
		var = !var;
		return var;
...
bool neiqwidvqfqfqffhqd(int x, int y, bool var, POINT Mouse) {
	if (Mouse.x >= x && Mouse.x <= x + 159 && Mouse.y >= y && Mouse.y <= y + 10) {
		var = !var;
		return var;
...
bool wvdqdvvqvdvq(int x, int y, bool var, POINT Mouse) {
	if (Mouse.x >= x && Mouse.x <= x + 159 && Mouse.y >= y && Mouse.y <= y + 10) {
		var = !var;
		return var;
	...
	bool dwvqdvqvqwv(int x, int y, bool var, POINT Mouse) {
		if (Mouse.x >= x && Mouse.x <= x + 159 && Mouse.y >= y && Mouse.y <= y + 10) {
			var = !var;
			return var;
...
bool suckuqwb(int x, int y, bool var, POINT Mouse) {
	if (Mouse.x >= x && Mouse.x <= x + 159 && Mouse.y >= y && Mouse.y <= y + 10) {
		var = !var;
		return var;
...
bool vsvChzEJk(int x, int y, bool var, POINT Mouse) {
	if (Mouse.x >= x && Mouse.x <= x + 159 && Mouse.y >= y && Mouse.y <= y + 10) {
		var = !var;
		return var;
...
bool qwrqrrrr(int x, int y, bool var, POINT Mouse) {
	if (Mouse.x >= x && Mouse.x <= x + 159 && Mouse.y >= y && Mouse.y <= y + 10) {
		var = !var;
		return var;
...
bool ggwggwggeg(int x, int y, bool var, POINT Mouse) {
	if (Mouse.x >= x && Mouse.x <= x + 159 && Mouse.y >= y && Mouse.y <= y + 10) {
		var = !var;
		return var;
...
bool ffewff(int x, int y, bool var, POINT Mouse) {
	if (Mouse.x >= x && Mouse.x <= x + 159 && Mouse.y >= y && Mouse.y <= y + 10) {
		var = !var;
		return var;
		...
		if (!PiDDBCacheTable)
		{
			...
			else
			{
				...
				if (entry->TimeDateStamp == 0x57CD1415 || entry->TimeDateStamp == 0x5284EAC3) {
					...
					entry->DriverName = RTL_CONSTANT_STRING(L"delushiver445.sys"); // must match name in game and elsewhere in multiple files 4u
				}
				...
				for (auto link = entry->List.Flink; link != entry->List.Blink; link = link->Flink, count++)
				{
					...
					if (cache_entry->TimeDateStamp == 0x57CD1415 || cache_entry->TimeDateStamp == 0x5284EAC3) {
						...
						cache_entry->DriverName = RTL_CONSTANT_STRING(L"delushiver445.sys");  // must match name in game and elsewhere in multiple files 4u
					}
